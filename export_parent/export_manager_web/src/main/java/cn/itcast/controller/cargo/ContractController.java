...
//        需要companyId
//        ContractExample构建查询条件     where company_id=?  order by create_time desc
        ...
        criteria.andCompanyIdEqualTo(companyId);  // 相当于   company_id=?
...
//        4、普通员工只能看自己的     xiaowang xiaoli  销售部北京分部的普通员工  where company_id=? and create_by = '当前登录人id'
    ...
    public String edit(Contract contract){
        if(StringUtils.isEmpty(contract.getId())){
            ...
            contract.setCreateDept(createDept);
...
//        2、文件的mime类型  常见的文件可以省略
    ...
    @RequestMapping(value = "/print",name = "跳转到导出出货表界面")
    public String print(){
    ...
    public String submit(String id){
        //将此id对应的数据状态修改成1
        contractService.updateState(id);
        return "redirect:/cargo/contract/list.do";
    }
}
