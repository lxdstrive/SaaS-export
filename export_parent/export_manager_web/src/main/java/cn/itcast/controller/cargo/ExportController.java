...
public class ExportController extends BaseController {
    ...
    @RequestMapping(value = "/contractList",name = "展示待报运的购销合同列表")
    public String contractList(@RequestParam(value = "page", defaultValue = "1") int page, @RequestParam(value = "pageSize", defaultValue = "10") int size) {
        ...
        example.createCriteria().andStateEqualTo(1).andCompanyIdEqualTo(companyId);
    ...
    public String edit(Export export){
        if (StringUtils.isEmpty(export.getId())){
            ...
            export.setState(0);//新增的报运单状态为草稿，对应的值为0
            export.setCreateTime(new Date());
            export.setCreateBy(createBy);
            export.setCreateDept(createDept);//在细颗粒度控制时需要
...
//        获取数据
    ...
    public void exportPdf(String id) throws Exception{
        ...
        Export export = exportService.findById(id);
        List<ExportProduct> exportProducts = exportProductService.findByExportId(id);//2、此报运单下的货物数据
        //将对象转为map数组
        Map<String, Object> params = BeanMapUtils.beanToMap(export);
